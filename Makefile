...
help:
	...
	@echo "  make test-e2e         - Run e2e tests (requires live Microsoft 365)"
...
test-e2e:
	O365_E2E=1 uv run pytest tests/e2e/ -v -m e2e

test-cov:
	uv run pytest tests/ -m "not e2e" --cov=src/office365_blade_mcp --cov-report=term-missing -v
...
type-check:
	uv run mypy src/office365_blade_mcp
